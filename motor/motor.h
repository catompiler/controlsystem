...
// По-умолчанию.
#define MOTOR_S_U_NOM_DEFAULT IQ15(6000)
...
// Минимум.
#define MOTOR_R_U_NOM_MIN IQ15(1)
// Максимум.
#define MOTOR_R_U_NOM_MAX IQ15(1000)
// По-умолчанию.
#define MOTOR_R_U_NOM_DEFAULT IQ15(50)

// Номинальный ток ротора.
// Минимум.
#define MOTOR_R_I_NOM_MIN IQ15(1)
// Максимум.
#define MOTOR_R_I_NOM_MAX IQ15(10000)
// По-умолчанию.
#define MOTOR_R_I_NOM_DEFAULT IQ15(250)
...
struct _S_Motor {
    ...
    // Момент.
    reg_iq15_t r_s_M_base; //!< Базовый момент.
    reg_iq24_t r_s_M_base_inv; //!< Базовый момент, инвертированное значение.
    // Момент инерции.
    reg_iq24_t r_s_J_base; //!< Базовый момент инерции.
    reg_iq7_t r_s_J_base_inv; //!< Базовый момент инерции, инвертированное значение.
    // Магнитный поток.
    reg_iq24_t r_s_Psi_base; //!< Базовое потокосцепление.
    reg_iq24_t r_s_Psi_base_inv; //!< Базовое потокосцепление, инвертированное значение.
    // Сопротивление.
    reg_iq24_t r_s_R_base; //!< Базовое сопротивление.
    reg_iq24_t r_s_R_base_inv; //!< Базовое сопротивление, инвертированное значение.
    // Индуктивность.
    reg_iq24_t r_s_L_base; //!< Базовая индуктивность.
    reg_iq15_t r_s_L_base_inv; //!< Базовая индуктивность, инвертированное значение.
    // Ёмкость.
//    reg_iq24_t r_s_C_base; //!< Базовая ёмкость.
//    reg_iq15_t r_s_C_base_inv; //!< Базовая ёмкость, инвертированное значение.
    // Ротор.
    // Напряжение.
    reg_iq15_t r_r_U_base; //!< Базовое напряжение.
    reg_iq24_t r_r_U_base_inv; //!< Базовое напряжение, инвертированное значение.
    // Ток.
    reg_iq15_t r_r_I_base; //!< Базовый ток.
    reg_iq24_t r_r_I_base_inv; //!< Базовый ток, инвертированное значение.
    // Мощность.
    reg_iq7_t r_r_P_base; //!< Базовая мощность.
    reg_iq24_t r_r_P_base_inv; //!< Базовая мощность, инвертированное значение.
    // Коэффициенты преобразования.
    // Статор.
    iq24_t r_s_k_U_mains_to_mot; //!< Напряжение относительно сети в напряжение относительно статора двигателя.
    iq24_t r_s_k_I_mains_to_mot; //!< Ток относительно сети в ток относительно статора двигателя.
    iq24_t r_s_k_I_mot_to_mains; //!< Ток относительно статора двигателя в ток относительно сети.
    // Ротор.
    iq24_t r_r_k_U_mains_to_mot; //!< Напряжение относительно сети в напряжение относительно ротора двигателя.
    iq24_t r_r_k_I_mains_to_mot; //!< Ток относительно сети в ток относительно ротора двигателя.
    iq24_t r_r_k_I_mot_to_mains; //!< Ток относительно ротора двигателя в ток относительно сети.
    // Методы.
    METHOD_INIT(M_motor);
...
#define MOTOR_DEFAULTS {\
        ...
        /* Ротор */\
        MOTOR_R_U_NOM_DEFAULT, /* p_U_nom */\
        MOTOR_R_I_NOM_DEFAULT, /* p_I_nom */\
        /* Регистры */\
        /* Общие двигателя */\
        ...
        /* Коэффициенты преобразования */\
        /* Статор */\
        0, /* r_s_k_U_mains_to_rot */\
        0, /* r_s_k_I_mains_to_rot */\
        0, /* r_s_k_I_rot_to_mains */\
        /* Ротор */\
        0, /* r_r_k_U_mains_to_rot */\
        0, /* r_r_k_I_mains_to_rot */\
        0, /* r_r_k_I_rot_to_mains */\
        /* Методы */\
