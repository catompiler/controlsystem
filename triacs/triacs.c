...
void TRIACS_MAINS_A_MOD_TIM_IRQ_Handler(void)
{
...
static void triacs_mains_mod_tim_init(M_triacs* tr)
{
    ...
    // configure.
    TRIACS_MAINS_A_MOD_TIM_CCU8_CC8->PSC = CCU_MOD_PRESCALER;
    TRIACS_MAINS_A_MOD_TIM_CCU8_CC8->PRS = CCU_MOD_PERIOD - 1;
    TRIACS_MAINS_A_MOD_TIM_CCU8_CC8->CR1S = 0;
    TRIACS_MAINS_A_MOD_TIM_CCU8_CC8->CR2S = CCU_MOD_PERIOD >> 1;
    TRIACS_MAINS_A_MOD_TIM_CCU8_CC8->CHC = CCU8_CC8_CHC_ASE_Msk; // Ассимметричный режим.
    TRIACS_MAINS_A_MOD_TIM_CCU8_CC8->TC = CCU8_CC8_TC_TSSM_Msk; // Однократный запуск.
    // shadow transfer.
    TRIACS_MAINS_A_MOD_TIM_CCU8->GCSS = TRIACS_MAINS_A_MOD_TIM_SHADOW_TRANSFER_Msk;
...
//    TRIACS_MAINS_A_MOD_TIM_CCU8_CC8->SWR = CCU8_CC8_SWR_RPM_Msk;
    ...
    // idle.
    TRIACS_MAINS_A_MOD_TIM_CCU8->GIDLC = TRIACS_MAINS_A_MOD_TIM_IDLE_CLR_Msk;
}
...
static int triacs_mains_mod_tim_start(M_triacs* tr)
{
    ...
    // start.
    TRIACS_MAINS_A_MOD_TIM_CCU8_CC8->TCSET = CCU8_CC8_TCSET_TRBS_Msk;
...
static int triacs_mains_mod_tim_stop(M_triacs* tr)
{
    ...
    // stop.
    TRIACS_MAINS_A_MOD_TIM_CCU8_CC8->TCCLR = CCU8_CC8_TCCLR_TRBC_Msk;
...
static void triacs_mains_mod_tim_deinit(M_triacs* tr)
{
    ...
    TRIACS_MAINS_A_MOD_TIM_CCU8->GIDLS = TRIACS_MAINS_A_MOD_TIM_IDLE_SET_Msk;
}
...
static void triacs_mains_out_a_tim_init(M_triacs* tr)
{
    ...
    // output.
    TRIACS_MAINS_A_OUT_TIM_CCU8_CC8->CHC = //CCU8_CC8_CHC_ASE_Msk | // Ассимметричный режим.
                                           CCU8_CC8_CHC_OCS1_Msk | CCU8_CC8_CHC_OCS3_Msk; // Инверсия сигналов.

    // shadow transfer.
    TRIACS_MAINS_A_OUT_TIM_CCU8->GCSS = TRIACS_MAINS_A_OUT_TIM_SHADOW_TRANSFER_Msk;

    // irqs.
    TRIACS_MAINS_A_OUT_TIM_CCU8_CC8->INTE = 0;//CCU8_CC8_INTE_PME_Msk;
    TRIACS_MAINS_A_OUT_TIM_CCU8_CC8->SRS = TRIACS_MAINS_A_OUT_TIM_SR << CCU8_CC8_SRS_POSR_Pos;
    // clear.
    TRIACS_MAINS_A_OUT_TIM_CCU8_CC8->SWR = CCU8_CC8_SWR_RPM_Msk;
...
static void triacs_mains_setup_fire(M_triacs* tr)
{
    ...
    TRIACS_MAINS_A_MOD_TIM_CCU8_CC8->CR1S = start_ticks;
    TRIACS_MAINS_A_MOD_TIM_CCU8_CC8->CR2S = end_ticks;
    // shadow transfer.
    TRIACS_MAINS_A_MOD_TIM_CCU8->GCSS = TRIACS_MAINS_A_MOD_TIM_SHADOW_TRANSFER_Msk;
}
...
static void triacs_mains_fire(M_triacs* tr)
{
    ...
    if(tr->in_control[PHASE3_CONTROL_AB]) triacs_mains_out_ap_tim_out_enable(tr);
    else triacs_mains_out_ap_tim_out_disable(tr);

    if(tr->in_control[PHASE3_CONTROL_AC]) triacs_mains_out_an_tim_out_enable(tr);
