...
def print_angle_def(angle, base):
    ...
    print("#define IQ24_CORDIC_ANGLE_PU_%d 0x%08x" % (angle, round(angle / 360.0 * base)));
...
def gen_tan_table():
    ...
    print("#define IQ24_CORDIC_GAIN 0x%08x\n" % (gain_iq));
    ...
    print("#define IQ24_CORDIC_TAN_TABLE_SIZE %u\n" % (table_size));
    print("//! Таблица значений тангенсов.");
    print("static const int32_t iq24_cordic_tan_table[IQ24_CORDIC_TAN_TABLE_SIZE] = {\n    ", end='');
    tan_val = 1.0;
    for i in range(0, table_size):
...
def main():
    print("#ifndef IQ24_TAN_TABLE_H");
    print("#define IQ24_TAN_TABLE_H");
    print("\n#include <stdint.h>\n\n");
