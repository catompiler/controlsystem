...
/**
 * Устанавливает аппаратный CS для обмена данными с EEPROM.
 * @param eeprom EEPROM.
 */
ALWAYS_INLINE static void m95x_set_hw_sel(m95x_t* eeprom, uint16_t sel)
{
#if defined(SPI_BUS_HW_SEL) && SPI_BUS_HW_SEL == 1
    spi_bus_set_hw_sel(eeprom->spi, sel);
#endif
}
...
{
    if(eeprom->ce_gpio) gpio_reset(eeprom->ce_gpio, eeprom->ce_pin_sel_msk);
    else m95x_set_hw_sel(eeprom, eeprom->ce_pin_sel_msk);
}
...
{
    if(eeprom->ce_gpio) gpio_set(eeprom->ce_gpio, eeprom->ce_pin_sel_msk);
    else m95x_set_hw_sel(eeprom, 0);
}
...
err_t m95x_init(m95x_t* eeprom, m95x_init_t* eeprom_init)
{
    ...
    eeprom->ce_pin_sel_msk = eeprom_init->ce_pin_sel_msk;
...
bool m95x_done(m95x_t* eeprom)
{
    return future_done(&eeprom->future);
}
