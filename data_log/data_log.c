...
static status_t recalc_values(M_data_log* dlog)
{
    ...
    for(i = 0; i < DATA_LOG_CH_COUNT; i ++){
        ...
        if(ch_par->p_enabled && (reg = regs_find(ch_par->p_reg_id))){
            ch_dat->ptr = reg_data(reg);
        }else{
            ch_dat->ptr = NULL;
        }
    }
...
METHOD_CALC_IMPL(M_data_log, dlog)
{
    ...
    for(i = 0; i < DATA_LOG_CH_COUNT; i ++){
        ...
        if(ch_par->p_enabled && ch_dat->ptr != NULL){
            val = *((data_log_value_t*)ch_dat->ptr);
        }else{
            val = 0;
        }

        ch_dat->data[dlog->m_put_index] = val;
    }

    uint32_t count = dlog->m_count;
    uint32_t put_index = dlog->m_put_index;
    uint32_t get_index = dlog->m_get_index;
    ...
    dlog->m_count = count;
    dlog->m_get_index = get_index;
    dlog->m_put_index = put_index;
}
